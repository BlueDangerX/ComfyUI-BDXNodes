...
class BDXNodesDialog extends ComfyDialog {
	static instance = null;

	static getInstance() {
		if (!BDXNodesDialog.instance) {
			BDXNodesDialog.instance = new BDXNodesDialog(app);
		}

		return BDXNodesDialog.instance;
	}
	...
	constructor() {
		if (BDXNodesDialog.instance) {
			throw new Error("Use BDXNodesDialog.getInstance() to get an instance of this class.");
		}
		...
		this.eventHandlers = {
			...
			pointerup: (event) => BDXNodesDialog.handlePointerUp(event),
			...
			documentPointerMove: (event) => BDXNodesDialog.getInstance().handlePointerMove(event),
		...
		this.points = []; // Array to store the points of the lasso
		this.antsOffset = 0; // Offset for the marching ants effect
	...
	setlayout(imgCanvas, maskCanvas) {
		...
		var copyButton = this.createRightButton("Copy", () => {
			// this.zoomLevel = 1;  // Reset zoom level to 1
			// this.childContainer.style.transform = `scale(${this.zoomLevel})`;  // Resetting zoom level in DOM
			// this.childContainer.style.left = this.left;
			// this.childContainer.style.top = this.top;
		...
		var pastaButton = this.createRightButton("Pasta", () => {
			// this.zoomLevel = 1;  // Reset zoom level to 1
			// this.childContainer.style.transform = `scale(${this.zoomLevel})`;  // Resetting zoom level in DOM
			// this.childContainer.style.left = this.left;
			// this.childContainer.style.top = this.top;
			...
			// const contextMenu = document.getElementById('canvasContextMenu');
			// contextMenu.style.display = 'none';
			this.pasteCopiedArea(this.previousMousePosition.x, this.previousMousePosition.y);
			// self.close();
		});
		...
		rightGroup.appendChild(pastaButton);
	...
	show() {
		...
		if (ComfyApp.clipspace_return_node) {
			this.saveButton.innerText = "Interpolate";
		}
		else {
	...
	handleMouseMove(event) {
		this.cursorX2 = event.clientX;
		this.cursorY2 = event.clientY;
	}
	...
	handleKeyDown(event) {
		const self = BDXNodesDialog.instance;
	...
	static handlePointerUp(event) {
		...
		const instance = BDXNodesDialog.instance;
		...
		if (BDXNodesDialog.instance.currentUndoStack.length > 0) {
			BDXNodesDialog.instance.currentUndoStack.splice(0, BDXNodesDialog.instance.currentUndoStack.length);
		}
	...
	// 				drawingContext.moveTo(controlPoint1.x, controlPoint1.y);
	...
	draw_move(self, event) {
		...
		// if (self.lastx !== null && self.lasty !== null) {
		//     drawingContext.beginPath();
		//     drawingContext.strokeStyle = this.brushColor; // Set a solid color for the lasso line
		//     drawingContext.lineWidth = 1; // Set a constant line width for the lasso
		//     drawingContext.moveTo(self.lastx, self.lasty);
		...
		if (self.lastx !== null && self.lasty !== null) {
			// Store the points instead of drawing immediately
			self.points.push({ x: self.lastx, y: self.lasty });
			self.lastx = x;
			self.lasty = y;
		}
	}
	...
	drawMarchingAnts() {
		const drawingContext = this.isGreyscale === 'rgb' ? this.imgCtx : this.maskCtx;
		drawingContext.clearRect(0, 0, drawingContext.canvas.width, drawingContext.canvas.height);
		drawingContext.setLineDash([4, 4]); // Dashed line pattern
		drawingContext.lineDashOffset = -this.antsOffset; // Apply the offset
	...
	updateMarchingAntsOffset() {
		this.antsOffset = (this.antsOffset + 1) % 8; // Update the offset
	}

	startMarchingAntsAnimation() {
		const animate = () => {
			this.drawMarchingAnts();
			this.updateMarchingAntsOffset();
	...
	handlePointerDown(self, event) {
		...
		if (!skipDrawing) {
			...
			const drawingContext = this.isGreyscale === 'rgb' ? this.imgCanvas.getContext('2d', { willReadFrequently: true }) : this.maskCanvas.getContext('2d', { willReadFrequently: true });
			...
			console.log(`handlePointerDown   mx: ${event.clientX}| my: ${event.clientY}`)
			console.log(`handlePointerDown    x: ${x} | y: ${y}`)
	...
	// copySelectedArea() {
	// 	const path = this.createLassoPath();
	// 	const bounds = path.getBounds(); // Assuming createLassoPath or similar method provides a way to get bounds
	
	// 	// Create an offscreen canvas of the size of the bounding box
	// 	const offScreenCanvas = document.createElement('canvas');
	// 	offScreenCanvas.width = bounds.width;
	// 	offScreenCanvas.height = bounds.height;
	// 	const offScreenCtx = offScreenCanvas.getContext('2d');
	
	// 	// Clip the path on the offscreen context
	// 	offScreenCtx.save();
	// 	offScreenCtx.clip(path);
	
	// 	// Draw only the part of the image inside the bounding box
	// 	offScreenCtx.drawImage(this.imgCtx.canvas, bounds.x, bounds.y, bounds.width, bounds.height, 0, 0, bounds.width, bounds.height);
	// 	offScreenCtx.restore();
	
	// 	this.copiedImage = offScreenCanvas;
	// }

	copySelectedArea() {
		const path = this.createLassoPath();
		const offScreenCanvas = document.createElement('canvas');
		offScreenCanvas.width = this.imgCtx.canvas.width;
		offScreenCanvas.height = this.imgCtx.canvas.height;
		const offScreenCtx = offScreenCanvas.getContext('2d');
		const bounds = this.createBoundingBox(this.points);
		offScreenCtx.save();
		offScreenCtx.clip(path);

		// const canvasCoords = this.getCanvasCoordinates(this.cursorX2, this.cursorY2);

		// offScreenCtx.drawImage(this.imgCtx.canvas, 0, 0);
		// offScreenCtx.drawImage(this.imgCtx.canvas, bounds.x, bounds.y, bounds.width, bounds.height, canvasCoords.x, canvasCoords.y, bounds.width, bounds.height);
		this.imgCtx.restore();
	...
	// 			const {x, y } = getActualCoordinates({clientX: this.cursorX, clientY: this.cursorY}, this.maskCanvas, this.zoomLevel);
	// 			console.log(`x: ${x} y: ${y}`)
	//         this.imgCtx.drawImage(this.copiedImage,  x,  y);
	//     }
	// }

	pasteCopiedArea(mouseX, mouseY) {
		if (this.copiedImage) {
			...
			console.log(`Pasting at x: ${canvasCoords.x}, y: ${canvasCoords.y}`);
			this.imgCtx.setTransform(1, 0, 0, 1, 0, 0);
			this.imgCtx.drawImage(this.copiedImage, canvasCoords.x, canvasCoords.y);
			
			// this.imgCtx.drawImage(this.copiedImage, 90, 62);

		} else {
			console.error("No copied image found for pasting");
		}
	}
	...
	createBoundingBox(points) {
		...
		// Iterate over the points to find min and max values
	...
	getCanvasCoordinates(screenX, screenY) {
		...
		// console.log(rect)
		// const event = { clientX: screenX, clientY: screenY }
		// const { x, y } = getActualCoordinates(event, this.isGreyscale === 'rgb' ? this.imgCanvas : this.maskCanvas, this.zoomLevel);
		// console.log(`getCanvasCoordinates   mx: ${event.clientX}| my: ${event.clientY}`)
		// console.log(`getCanvasCoordinates    x: ${x} | y: ${y}`)

		// const rect = this.childContainer.getBoundingClientRect();
		// const style = window.getComputedStyle(this.childContainer);
		// const matrix = new DOMMatrix(style.transform);
		// console.log(rect, style, matrix);

		// console.log(`screenX - rect.left: ${screenX} - ${rect.left} = ${screenX - rect.left} | ${(screenX - rect.left) * this.zoomLevel}`);
		// console.log(`screenY - rect.top: ${screenY} - ${rect.top} = ${screenY - rect.top} | ${(screenY - rect.top) * this.zoomLevel}`);
		console.log(`zoom level: ${this.zoomLevel}`)
		// const calcX = (screenX - rect.left) / (rect.right - rect.left) * this.imgCanvas.width
		// const calcY = (screenY - rect.top) / (rect.bottom - rect.top) * this.imgCanvas.height;
		// const calcX = (screenX - rect.left) / matrix.a - parseFloat(style.left);
		// const calcY = (screenY - rect.top) / matrix.d - parseFloat(style.top);

		const calcX = (screenX - rect.left) / this.zoomLevel;
		const calcY = (screenY - rect.top) / this.zoomLevel;

		console.log(`getCanvasCoordinates calc X:${calcX} and Y: ${calcY}`)
		return {
			x: calcX,
			y: calcY
		};
	...
	async copySelected() {
		...
		try {
			...
			// Create a new canvas to draw the clipped area
			const clippedCanvas = new OffscreenCanvas(this.originalWidth, this.originalHeight);
			const clippedCtx = clippedCanvas.getContext('2d');
			clippedCtx.drawImage(offscreen, 0, 0);

			// Export the clipped area as an image file
			const blob = await clippedCanvas.convertToBlob();
			const clipboardItem = new ClipboardItem({ "image/png": blob });
			await navigator.clipboard.write([clipboardItem]);

			console.log("Copied to clipboard");
		} catch (error) {
			console.error("Error copying selected area: ", error);
		}
	}
	...
	async save_mask(imgFilenameFromRGB) {
		...
		const worker = new Worker("../../extensions/BDXNodes/imageProcessorWorker.js");
...
app.registerExtension({
	name: "Comfy.BDXNodes.SelectCopyPasta",
	init(app) {
		const callback =
			function () {
				let dlg = BDXNodesDialog.getInstance();
				dlg.show();
			};

		const context_predicate = () => ComfyApp.clipspace && ComfyApp.clipspace.imgs && ComfyApp.clipspace.imgs.length > 0
		ClipspaceDialog.registerButton("BDXNodes", context_predicate, callback);
	},
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		if (nodeData.output.includes("MASK") && nodeData.output.includes("IMAGE")) {
			addMenuHandler(nodeType, function (_, options) {
				options.unshift({
					content: "Open in SelectCopyPasta",
					callback: () => {
